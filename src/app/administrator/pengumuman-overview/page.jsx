...
export default function Pengumuman() {
  ...
  const onDelete = async (id) => {
    try {
      const response = await request.delete(`/pengumuman/${id}`);
      toast.success("Data Pengumuman berhasil dihapus");
  ...
  const fetchAllPengumuman = useCallback(
    async (searchQuery = "", filterType = "") => {
      ...
      try {
        ...
        const response = await request.get("/pengumuman", { params });
  ...
  useEffect(() => {
    fetchAllPengumuman(query);
  }, [query, fetchAllPengumuman]);
  ...
  return (
    <section className="w-full max-w-[1200px] mx-auto px-4 md:px-0 mt-16 md:mt-0">
      <div className="flex flex-col md:flex-row items-center justify-between mb-4">
        ...
        <div className="flex mt-2 md:mt-0">
          <Button
            variant="default"
            icon={Plus}
            onClick={() => router.push("/administrator/create-pengumuman")}
          >
            Create
          </Button>
        </div>
      ...
      <Modal
        ...
        data={selectedRow}
        fields={modalFields}
        apiPath={selectedRow ? `/pengumuman/${selectedRow.id}` : ""}
        method="PATCH"
        mode={modalMode}
        onUpdate={(updatedRow) => {
          fetchAllPengumuman();
