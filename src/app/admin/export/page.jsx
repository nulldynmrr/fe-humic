...
export default function ExportXlsxToJsonPage() {
        ...
        const mappedJson = rawJson.map((row) => {
          ...
          apiKeys.forEach((key) => {
            if (row[key]) {
              ...
              obj[key] = ""; // default kosong jika tidak ada
            }
          });
          return obj;
        });

        // Filter hanya data yang memiliki title
  ...
  const handlePostData = useCallback(async () => {
    ...
    try {
      ...
      // Kirim data satu per satu seperti di import page
        ...
        try {
          ...
          if (item.image_path && item.image_path.trim() !== "") {
            try {
              ...
              console.warn("Gagal fetch image:", item.image_path, imgError);
              // Continue tanpa image jika gagal fetch
            }
          }

          await request.post("/pengumuman", formData, {
            "Content-Type": "multipart/form-data",
          });

          successCount++;
        } catch (itemError) {
          console.error("Error posting item:", item, itemError);
          errorCount++;
        }
      }
      ...
      if (successCount > 0) {
        alert(
          `✅ Berhasil mengirim ${successCount} data ke server!${
        ...
        alert("❌ Gagal mengirim semua data ke server.");
      }
    } catch (error) {
